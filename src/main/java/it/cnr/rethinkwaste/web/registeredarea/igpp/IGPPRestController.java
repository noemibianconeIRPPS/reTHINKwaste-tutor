...
public class IGPPRestController {

    private static DecimalFormat df = new DecimalFormat("#.##");
    private static DecimalFormat dfFourDecimalPlaces = new DecimalFormat("#.####");
    private static DecimalFormat dfZeroDecimalPlaces = new DecimalFormat("#");
    ...
    @GetMapping(value = "/downloadIGPPreport")
    public void downloadIGPPreport(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if(igppModule.isPresent()) {
            ...
            String pathToDownloadFile = reportCreation.createIGPPReport(igppModule.get(), reportService, ((CustomUserDetails) user).getUser());
    ...
    @GetMapping(value = "/downloadIGPPassessment")
    public void downloadIGPPassessment(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if(igppModule.isPresent()) {
            ...
            String pathToDownloadFile = assessmentCreation.createAssessment(igppModule.get());
            File file = new File(pathToDownloadFile);
    ...
    public Response saveStep(@RequestBody String obj) {
        ...
        if(user.getModuleInstanceList().stream().filter(m -> m.getModule().getId() == 4).findFirst().isPresent()) {
            moduleInstance = user.getModuleInstanceList().stream().filter(m -> m.getModule().getId() == 4).findFirst().get();
        }
        else {
            moduleInstance = new ModuleInstance();
            moduleInstance.setModule(moduleService.findById(4L));
    ...
    public ModuleInstance percentageCalc(ModuleInstance moduleInstance) {
        ...
        dfFourDecimalPlaces.setDecimalFormatSymbols(dfs);
        ...
        if(percentage == Double.valueOf("100")) {
            for(Category category : moduleInstance.getCategoryCategoryInstanceSortedMap().keySet()) {
                ...
                for(Question q : categoryInstance.getQuestionAnswerSortedMap().keySet()) {
                    double questionScore = categoryInstance.getQuestionAnswerSortedMap().get(q).getScore();
                    score += questionScore;
                    categoryScore += questionScore;

                    score = Double.parseDouble(dfFourDecimalPlaces.format(score));
                    categoryScore = Double.parseDouble(dfFourDecimalPlaces.format(categoryScore));
                }
                categoryInstance.setCategoryScore(categoryScore);
                categoryInstanceService.save(categoryInstance);
            }
        }
        score = Double.parseDouble(dfZeroDecimalPlaces.format(score));
